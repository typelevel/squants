 ...
 *
 * [[Price]]s represent a Ratio between Money and some other Quantity.  Prices can be created from
 ...
 * }}}
 *
 * [[CurrencyExchangeRate]]s represent conversion rates between currencies.
 * Use them to explicitly convert Money values in one currency to values in another.
 *
 * [[MoneyContext]] provide the implicit context necessary to perform cross-currency operations
...
package object market {
  ...
  lazy val defaultMoneyContext: MoneyContext[Double] = MoneyContext(USD, defaultCurrencySet, Nil)

  class NoSuchExchangeRateException(val s: String) extends Exception

  case class NoSuchCurrencyException(code: String, fxContext: MoneyContext[_]) extends Exception(
