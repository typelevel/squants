 ...
 */
case class MoneyContext[A](
    defaultCurrency: Currency,
    currencies: Set[Currency],
    rates: Seq[CurrencyExchangeRate[A]],
    allowIndirectConversions: Boolean = true)(implicit num: Numeric[A]) {
   ...
   * @return
   */
  def directRateFor(curA: Currency, curB: Currency): Option[CurrencyExchangeRate[A]] = {
    rates.find(r ⇒
   ...
   * @return
   */
  def indirectRateFor(curA: Currency, curB: Currency): Option[CurrencyExchangeRate[A]] = {
    ...
    directRateFor(curA, curB) match {
      ...
      case _ ⇒
        ...
        curs.headOption match {
          case Some(cur) ⇒ Some(CurrencyExchangeRate(convert(cur(num.one), curA), convert(cur(num.one), curB)))
          case None      ⇒ None
        }
    }
  }
   ...
   */
  def convert(money: Money[A], currency: Currency): Money[A] = {
   ...
   * @throws NoSuchExchangeRateException when no exchange rate is available
   */
  def add(moneyA: Money[A], moneyB: Money[A]): Money[A] = moneyA.currency(moneyA.value + convert(moneyB, moneyA.currency).value)
   ...
   * @throws NoSuchExchangeRateException when no exchange rate is available
   */
  def subtract(moneyA: Money[A], moneyB: Money[A]): Money[A] = moneyA.currency(moneyA.value - convert(moneyB, moneyA.currency).value)
   ...
   * @return
   */
  def divide(moneyA: Money[A], moneyB: Money[A]): A = moneyA.value / convert(moneyB, moneyA.currency).value
   ...
   * @throws NoSuchExchangeRateException when no exchange rate is available
   */
  def compare(moneyA: Money[A], moneyB: Money[A]): Int =
    if (moneyA.value > convert(moneyB, moneyA.currency).value) 1
    else if (moneyA.value < convert(moneyB, moneyA.currency).value) -1
    else 0
  ...
  /**
    ...
    * @return
    */
  def withAdditionalCurrencies(additionalCurrencies: Set[Currency]): MoneyContext[A] = copy (currencies = currencies ++ additionalCurrencies)
  ...
  /**
    ...
    * @return
    */
  def withExchangeRates(rates: List[CurrencyExchangeRate[A]]): MoneyContext[A] = copy(rates = rates)
}
