...
final case class Volume[A: Numeric : Converter] private [space] (value: A, unit: VolumeUnit) extends Quantity[A, Volume.type] {
...
object Volume extends Dimension("Volume") {
  ...
  implicit class VolumeCons[A: Numeric : Converter](a: A) {
...
abstract class VolumeUnit(val symbol: String, val conversionFactor: ConversionFactor) extends UnitOfMeasure[Volume.type] {
  ...
  override def apply[A: Numeric : Converter](value: A): Volume[A] = Volume(value, this)
}
