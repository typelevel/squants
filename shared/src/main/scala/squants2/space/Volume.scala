...
final case class Volume[A: QNumeric] private [space] (value: A, unit: VolumeUnit) extends Quantity[A, Volume.type] {
...
object Volume extends Dimension("Volume") {
  ...
  implicit class VolumeCons[A: QNumeric](a: A) {
...
abstract class VolumeUnit(val symbol: String, val conversionFactor: Double) extends UnitOfMeasure[Volume.type] {
  override def dimension: Volume.type = Volume
  override def apply[A: QNumeric](value: A): Volume[A] = Volume(value, this)
}
