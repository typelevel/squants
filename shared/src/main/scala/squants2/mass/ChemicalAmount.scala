...
final case class ChemicalAmount[A: QNumeric] private [mass] (value: A, unit: ChemicalAmountUnit) extends Quantity[A, ChemicalAmount.type] {
...
object ChemicalAmount extends BaseDimension("Chemical Amount", "N") {
  ...
  implicit class ChemicalAmountCons[A: QNumeric](a: A) {
...
abstract class ChemicalAmountUnit(val symbol: String, val conversionFactor: Double) extends UnitOfMeasure[ChemicalAmount.type] {
  ...
  override def apply[A: QNumeric](value: A): ChemicalAmount[A] = ChemicalAmount(value, this)
}
