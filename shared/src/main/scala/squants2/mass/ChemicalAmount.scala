...
final case class ChemicalAmount[A: Numeric : Converter] private [mass] (value: A, unit: ChemicalAmountUnit) extends Quantity[A, ChemicalAmount.type] {
...
object ChemicalAmount extends BaseDimension("Chemical Amount", "N") {
  ...
  implicit class ChemicalAmountCons[A: Numeric : Converter](a: A) {
...
abstract class ChemicalAmountUnit(val symbol: String, val conversionFactor: ConversionFactor) extends UnitOfMeasure[ChemicalAmount.type] {
  ...
  override def apply[A: Numeric : Converter](value: A): ChemicalAmount[A] = ChemicalAmount(value, this)
}
