...
final case class Mass[A: QNumeric] private [mass] (value: A, unit: MassUnit) extends Quantity[A, Mass.type] {
...
object Mass extends BaseDimension("Mass", "M") {
  ...
  implicit class MassCons[A: QNumeric](a: A) {
...
abstract class MassUnit(val symbol: String, val conversionFactor: Double) extends UnitOfMeasure[Mass.type] {
  ...
  override def apply[A: QNumeric](value: A): Mass[A] = Mass(value, this)
}
