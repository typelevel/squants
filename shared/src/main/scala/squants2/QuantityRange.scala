...
case class QuantityRange[A, Q[_] <: Quantity[_, Q]](lower: Quantity[A, Q], upper: Quantity[A, Q])(implicit num: Numeric[A]) {
   ...
   * @return
   */
  def times[B](multiple: B)(implicit f: B => A): QuantitySeries[A, Q] = {
  ...
  /** times */
  def *[B](multiple: B)(implicit f: B => A): QuantitySeries[A, Q] = times(multiple)
   ...
   * @return
   */
  def divide[B](that: B)(implicit f: B => A): QuantitySeries[A, Q] = divide(this.toQuantity / that)
  /** divide */
  def /[B](that: B)(implicit f: B => A): QuantitySeries[A, Q] = divide(that)
   ...
   * @return
   */
  def foreach[B, U](divisor: B)(op: QuantityRange[A, Q] => U)(implicit f: B => A): Unit = /(divisor).foreach(op)
   ...
   * @return
   */
  def map[B, C](divisor: B)(op: QuantityRange[A, Q] => C)(implicit f: B => A): Seq[C] = map(toQuantity / divisor)(op)
   ...
   * @return
   */
  def foldLeft[B, C](divisor: B, z: C)(op: (C, QuantityRange[A, Q]) => C)(implicit f: B => A): C = /(divisor).foldLeft[C](z)(op)
  /** foldLeft */
  def /:[B, C](divisor: B, z: C)(op: (C, QuantityRange[A, Q]) => C)(implicit f: B => A): C = foldLeft(divisor, z)(op)
   ...
   * @return
   */
  def foldRight[B, C](divisor: B, z: C)(op: (QuantityRange[A, Q], C) => C)(implicit f: B => A): C = /(divisor).foldRight[C](z)(op)
  /** foldRight */
  def :\[B, C](divisor: B, z: C)(op: (QuantityRange[A, Q], C) => C)(implicit f: B => A): C = foldRight(divisor, z)(op)

  /**
   * Increments the range's from and to values by an amount equal to the Quantity value of the range
   * @return
   */
  lazy val inc: QuantityRange[A, Q] = QuantityRange(lower + toQuantity, upper + toQuantity)
  /** inc */
  def ++(): QuantityRange[A, Q] = inc
   ...
   * @return
   */
  lazy val dec: QuantityRange[A, Q] = QuantityRange(lower - toQuantity, upper - toQuantity)
  /** dec */
  def --(): QuantityRange[A, Q] = dec
   ...
   * @return
   */
  def contains[B](that: QuantityRange[B, Q])(implicit f: B => A): Boolean = {
    val thatA = that.asNum[A]
   ...
   * @return
   */
  def partiallyContains[B](range: QuantityRange[B, Q])(implicit f: B => A): Boolean =
   ...
   * @return
   */
  def includes[B](that: QuantityRange[B, Q])(implicit f: B => A): Boolean = {
    val thatA = that.asNum[A]
    thatA.lower >= this.lower &&
      thatA.lower <= this.upper &&
      thatA.upper >= this.lower &&
   ...
   * @return
   */
  def partiallyIncludes[B](range: QuantityRange[B, Q])(implicit f: B => A): Boolean =
   ...
   * @return
   */
  def asNum[B: Numeric](implicit f: A => B): QuantityRange[B, Q] = QuantityRange(lower.asNum[B], upper.asNum[B])
   ...
   * @return
   */
  lazy val toSeq: Seq[Quantity[A, Q]] = Seq(lower, upper)

  /**
   * Return this Range's boundary values as List[A] or the two
   * @return
   */
  lazy val toList: List[Quantity[A, Q]] = List(lower, upper)
}
